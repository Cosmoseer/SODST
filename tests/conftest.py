...
from sodst.core import CentralBody, Satellite, Orbit, asymptote_anomaly
...
def orbit_factory_from_elements() -> Callable[[float, float, float, float, float, float, float], Orbit]:
    def _create(e: float = 0.0, rp: float = 50_000.0, raan: float = 0.0, i: float = 0.0,
                omega: float = 0.0, mu: float = 398_600.0, nu: float = 0.0) -> Orbit:
        return Orbit.from_orbital_elements(e, rp, raan, i, omega, mu, nu)
    return _create
...
def satellite_factory_from_elements(
    ...
    def _create(e: float = 0.0, rp: float = 50_000.0, raan: float = 0.0, i: float = 0.0,
                omega: float = 0.0, mu: float = 398_600.0, nu: float = 0.0) -> Satellite:
        orbit: Orbit = orbit_factory_from_elements(e, rp, raan, i, omega, mu, nu)
        r = orbit.position
        v = orbit.velocity
        central_body: CentralBody = central_body_factory(mu)
        return Satellite(r, v, central_body)
